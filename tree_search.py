        ...
        while self.open_nodes:
            ...
            for a in self.problem.domain.actions(node.state):
                ...
                if not node.in_parent(newstate) and node.depth + 1 < limit:
                    lnewnodes += [
                        SearchNode(newstate, node, node.depth + 1, node.cost + self.problem.domain.cost(node.state, a),
            ...
            if node.cost > self.most_cost_list[0].cost:
                self.most_cost_list = [node]

            if node.cost == self.most_cost_list[0]:
                self.most_cost_list.append(node)
