        ...
        while True:
            try:
                ...
                if len(walls) == 0:  # If walls are all destroyed
                    ...
                    if put_bomb:  # Run from bomb and wait for explosion
                        ...
                        if pos_enemy is not None:
                            if calc_distance(position, pos_enemy) < 3 and not put_bomb and on_same_line(position,pos_enemy,mapa):
                                key = attack()
                    ...
                    if len(enemies) != 0:  # If there are still enemies
                        if has_Ballooms(enemies):
                            ...
                            elif calc_distance(position, pos_enemy) < 4 and not put_bomb and on_same_line(position,pos_enemy,mapa):
                                key = attack()
                        else:
                            attack_distance = 3
                            key = astar_path(mapa.map, position, pos_enemy, True, enemies)
                            if calc_distance(position,pos_enemy) < attack_distance and not put_bomb and on_same_line(position,pos_enemy,mapa):
                                key = attack()
                ...
                ############################################### With Walls ####################################################
                    ...
                    print(run)
                    if put_bomb and run == True:  # Run from bomb and wait for explosion
                        key = astar_path(mapa.map, position, run_to, True, enemies)
                        if position == [1, 1]:
                            run = False
                            put_bomb = False
                    ...
                    if has_Oneals(enemies) and put_bomb == False:
                        attack_distance = 4
                ...
                if not put_bomb and not power_up_found:  # if bomb is not planted and power-up not found yet
                    key = astar_path(mapa.map, position, find_power_up(state), True,enemies)  # Get power-up
                    level = 2
...
def has_enemy(location,enemies):
    ...
    for enemy in enemies:
        ...
        if (x,y) == enemy or (x+1,y) == enemy or (x-1,y-1) == enemy or (x,y-1) == enemy or (x-1,y-1)==enemy or (x-1,y) == enemy or (x-1,y+1) == enemy or (x,y+1) == enemy or (x+1,y+1) == enemy:
            return True
    return False    
